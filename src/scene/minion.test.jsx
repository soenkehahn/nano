...
describe("Minion", () => {
  it("shows minion in svg context", () => {
    expect(wrapper().find("svg").find(MinionRender).exists()).toEqual(true);
  });
...
describe("Minions", () => {
  ...
  it("keeps moving unfocused minions", () => {
    ...
    wrapper()
      ...
      .simulate("click", toClickEvent({ x: 100, y: 0 }));
    step(2);
    expect(wrapper().find(MinionRender).at(0).props().position).toEqual({
      x: 0,
      y: 1,
    });
  });
  ...
  it("shows the status of a minion", () => {
    expect(wrapper().find("#minion-ui-0").text()).toContain("status: idle");
  });
  ...
  it("shows the status of moving minions", () => {
    ...
    wrapper()
      ...
      .simulate("click", toClickEvent({ x: 1, y: 0 }));
    step();
    expect(wrapper().find("#minion-ui-0").text()).toContain("status: moving");
  });

  describe("focus buttons", () => {
    it("shows a focus button", () => {
      expect(wrapper().find("#focusButton-0").exists()).toEqual(true);
    });
    ...
    describe("when there are two minions", () => {
      ...
      it("shows a focus button for the unfocused minion", () => {
        expect(wrapper().find("#focusButton-1").exists()).toEqual(true);
      });

      it("allows to switch focus to the unfocused minion", () => {
        wrapper().find("#focusButton-1").simulate("click");
      ...
      it("shows two move buttons", () => {
        ...
        expect(wrapper().find("#moveButton-1").exists()).toEqual(true);
      });
